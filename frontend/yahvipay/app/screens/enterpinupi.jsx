...
export default function enterpinupi() {
  ...
  const { upiId, amount, name } = useLocalSearchParams();
  ...
  const handleKeyPress = (e, index) => {
    ...
    const HandleSubmit = async() => {
        let result = await SecureStore.getItemAsync('token');
            console.log(result)
            try {
            const apiUrl = process.env.EXPO_PUBLIC_API_PROCESS_PAY_UPI_ID || '';
            console.log(apiUrl)
            const response = await fetch(apiUrl, {
                ...
                body : JSON.stringify({ token : result, upiId : upiId, amount : amount, passwordPin : pin  })
            })
            ...
            if(response.ok) {
                ...
                router.replace({ pathname : 'screens/success', params : { amount : amount, status : 'paid', name : name, transactionId : data }})
            }
            else{
                console.log('qq')
                router.replace({ pathname : 'screens/success', params : { amount : amount, status : 'failed', name : name, transactionId : data }  })
            }
            }
            catch(e) {
                console.log(e)
