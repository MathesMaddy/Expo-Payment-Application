...
export default function enterpin() {
  ...
  const { rechargeDth, amount, billType } = useLocalSearchParams();
  ...
  const HandleSubmit = async() => {
    ...
    try {
            const apiUrl = process.env.EXPO_PUBLIC_API_PROCESS_DTH_RECHARGE || '';
            ...
            const response = await fetch(apiUrl, {
              ...
              body : JSON.stringify({ token : result, dthNumber : rechargeDth, billType: billType, amount : amount, passwordPin : pin })
            })
            if(response.ok) {
              router.replace({ pathname : 'screens/success', params : { status : 'paid', name : billType  }})
            }
            else if(response.status === 406) {
              router.replace({ pathname : 'screens/success', params : { status : 'failed', name : billType  }})
            }
            else {
              router.replace({ pathname : 'screens/success', params : { status : 'failed', name : billType  }})
            }
