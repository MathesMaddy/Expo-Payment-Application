...
export default function enterpinupi() {
  ...
  const HandleSubmit = async() => {
    ...
    try {
            ...
            if(response.ok) {
              router.replace({ pathname : 'screens/success', params : { status : 'paid', amount : amount, transactionId : data, name : name }})
            }
            else if(response.status === 406) {
              
              router.replace({ pathname : 'screens/success', params : { status : 'failed', amount : amount, transactionId : data, name : name  }})
              //   setOtpEntered(false);
            }
            else {
              router.replace({ pathname : 'screens/success', params : { status : 'failed', amount : amount, transactionId : data, name : name  }})
