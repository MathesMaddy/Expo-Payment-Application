...
export default function enterpinupi() {
  ...
  const HandleSubmit = async() => {
    ...
    try {
            const apiUrl = process.env.EXPO_PUBLIC_API_PROCESS_CHANGE_PIN || '';
            ...
            if(response.ok) {
                console.log('qq')
              router.replace({ pathname : '/', params : { res : response.status }})
            }
  ...
  return (
    ...
    <View style={{ justifyContent : 'space-between', flexDirection : 'row', paddingTop : 10 }}>
        <Text style={{ color : 'white', fontSize : 20, fontFamily : 'bold' }}>Pin change</Text>
    </View>
    </View>
    <View>
      <View style={styles.inputContainer}>
        <Text style={styles.label}>Create new 6 digit PIN</Text>
