...
export default function enterpinbanktransfer() {
  ...
  const { bankAccountNumber, ifscCode, amount, name } = useLocalSearchParams();
  ...
  const HandleSubmit = async() => {
    ...
    try {
            const apiUrl = process.env.EXPO_PUBLIC_API_PROCESS_BANK_PAYMENT || '';
            ...
            const response = await fetch(apiUrl, {
              ...
              body : JSON.stringify({ token : result, bankAccountNumber : bankAccountNumber, ifscCode : ifscCode, amount : amount, pin : pin })
            })
            const data = await response.json()
            if(response.ok) {
              router.replace({ pathname : 'screens/success', params : { status : 'paid', amount : amount, message : 'Bank Transaction', name : name, transactionId : data }})
            }
            else if(response.status === 406) {
              
              router.replace({ pathname : 'screens/success', params : { status : 'failed', amount : amount, message : 'Bank Transaction', name : name, transactionId : data  }})
              //   setOtpEntered(false);
            }
            else {
              router.replace({ pathname : 'screens/success', params : { status : 'failed', amount : amount, message : 'Bank Transaction', name : name, transactionId : data  }})
