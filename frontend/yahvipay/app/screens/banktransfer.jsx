...
export default function Banktransfer() {
  ...
  useEffect(() => {
      const FetchRecentPay = async() => {
        try {
          ...
          if (token) {
            try {
              const apiUrl = process.env.EXPO_PUBLIC_API_PROCESS_CHECK_RECENT_BANK_TRANSFER || ''
  ...
  const HandleRecentPay = (accountNumber, ifsc) => {
        ...
        <Text style={styles.recentPayTitle}>Recent Pay</Text>
          <View style={styles.recentPayContainer}>
            { recentPay?.map((item, index) => (
              <TouchableOpacity key={index} onPress={() => HandleRecentPay(item.receiver, item.ifscCode)}>
                <View style={styles.recentPay}>
                  ...
                  <Text style={styles.recentPayName}>{item.receiverName}</Text>  
                </View>
              </TouchableOpacity>
            )) }
          </View>
